...
function btoa(a){
	return new Buffer.from(a, 'binary').toString("base64")
}
...
function passwdGen(staYear,stpYear){
	var y0=new Date(staYear,0,1,0,0,0,0).getTime()
	var y1=new Date(stpYear,11,31,0,0,0,0).getTime();
	re=[];
	var i=y0
	while(i<=y1){
		//console.log(dateFormat("YYYYmmdd",new Date(i)));
		i+=86400000;
		re.push(dateFormat("YYYYmmdd",new Date(i)))
	}
	return re;
}

function sub(idName,idPassword){
        var key = "rEybhlpaq09236H@fg712}3[";//文档中的key为32进制"ckV5YmhscGFxMDkyMzZIQGZnNzEyfTNb"
        username = idName;
        idName = DES3.encrypt(key,idName);
		print(idName)
        idPassword = DES3.encrypt(key,idPassword);
		print(idPassword)
		request({
            url: "http://zhsz.bjedu.cn/web/login/validate",
            method: 'POST',
            body:urlencode.stringify({'user':idName,'pass':idPassword,'save':0,sign:btoa(username)}),
			headers: {
